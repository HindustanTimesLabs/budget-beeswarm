...
function draw(){
  ...
  d3.csv("data.csv", type, function(error, data) {
    ...
    cell.append("circle")
        ...
        .style("fill", function(d) { if (d != undefined) return colorScale(d.data["sub-category"]); });
    ...
    function tipOn(d, i){
      ...
      $(".tip").append("<div class='ministry' style='color:" + colorScale(dat["sub-category"]) + "'>" + dat["sub-category"] + "</div>");
      $(".tip").append("<table></table>");
      $(".tip table").append("<thead><td>2016-17 (Rs)</td><td>2017-18 (Rs)</td><td>Change</td></thead>");
      $(".tip table").append("<tr class='data'><td>" + strings.numberLakhs(dat["2016-17 (RE)"]) + " cr</td><td>" + strings.numberLakhs(dat["2017-18 (BE)"]) + " cr</td><td>" + strings.numberDecimals(dat.pct * 100, 2) + "%</td></tr>");
